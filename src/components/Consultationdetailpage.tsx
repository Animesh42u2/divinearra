    ...
    <div style={{ background: CREAM, minHeight: '100vh', color: BROWN_TEXT, fontFamily: "'Inter', 'Segoe UI', sans-serif" }}>
      ...
      <style>{`
        ...
        @media (max-width: 900px) and (min-width: 641px) {
          .cp-hero-right { width: 240px; height: 240px; }
          .cp-hero { padding: 48px 32px 64px; }
        }
      ...
      <section className="cp-hero">
        <div className="cp-hero-anim cp-hero-left">
          <span style={{ border: '1px solid rgba(255,255,255,0.4)', borderRadius: 20, padding: '4px 14px', fontSize: 11, marginBottom: 16, display: 'inline-block', letterSpacing: '0.12em', fontWeight: 600 }}>
            ✦ Live Consultation with Aditya Narayan Panigrahi
          </span>
      ...
      <section style={{ maxWidth: 1100, margin: '0 auto', padding: 'clamp(3rem,6vw,5rem) clamp(1rem,3vw,1.5rem)' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(300px,100%), 1fr))', gap: 'clamp(2rem,5vw,4rem)', alignItems: 'center' }}>
          <div>
            ...
            <a href="#pricing" style={{ ...ctaBtn }}>Book Now</a>
          </div>
...
<section id="whats-inside" style={{
  ...
  <style>{`
    .kg { display:grid; grid-template-columns:1fr 1fr 1fr 1fr; border:2px solid #3a5070; position:relative; }
    .kg-house { border:1.5px solid #3a5070; padding:clamp(.4rem,.8vw,.65rem) clamp(.4rem,.8vw,.6rem); display:flex; flex-direction:column; min-height:clamp(80px,14vw,130px); }
    .kg-icon { width:clamp(24px,4vw,34px); height:clamp(24px,4vw,34px); border-radius:50%; background:linear-gradient(135deg,#8b4e0a,#c8791a); display:flex; align-items:center; justify-content:center; margin-bottom:.3rem; flex-shrink:0; box-shadow:0 2px 6px rgba(139,78,10,.4); }
    .kg-title { font-size:clamp(.55rem,1.1vw,.75rem); font-weight:700; color:#2a1200; line-height:1.25; margin:.15rem 0 .1rem; font-family:'Playfair Display',serif; flex-shrink:0; }
    ...
    .kg-center { grid-column:2/4; grid-row:2/3; position:relative; overflow:hidden; border:1.5px solid #3a5070 !important; }
    ...
    .kg-brand { font-size:clamp(.48rem,.9vw,.65rem); font-weight:700; color:#8b4e0a; font-family:'Playfair Display',serif; }
    .kg-item5 { grid-column:2/4; grid-row:3/4; }

    /* ── Roller: light wooden stick + round knobs ── */
    .roller-wrap { position:relative; width:100%; height:clamp(28px,4vw,40px); display:flex; align-items:center; z-index:2; margin:0; }
    .roller-stick {
    ...
    .roller-knob-l { left:clamp(-13px,-2vw,-9px); }
    .roller-knob-r { right:clamp(-13px,-2vw,-9px); }

    .parchment {
      background:linear-gradient(160deg,#f7ecc8 0%,#ead8a0 25%,#f2e0b0 55%,#e8d49a 100%);
      margin:0 14px;
      padding:clamp(.6rem,2vw,1.2rem) clamp(.6rem,2vw,1rem);
    ...
    <div style={{ maxWidth: 780, margin: '0 auto', position: 'relative' }}>
      ...
      <div className="parchment">
        <div className="kg">
          ...
          {/* Item 4 — bottom center spans 2 cols */}
...
<section style={{ background: CREAM, padding: 'clamp(3rem,6vw,6rem) clamp(1rem,3vw,1.5rem)', position: 'relative', overflow: 'hidden' }}>
  ...
  <style>{`
    ...
    .fw-cta-btn {
      ...
      margin-top: 0.9rem;
      box-shadow: 0 4px 16px rgba(200,121,26,0.35);
  ...
  <div style={{ maxWidth: 1100, margin: '0 auto', position: 'relative', zIndex: 1 }}>
    ...
    <div className="for-whom-grid">
      {consultation.forWhom.map((item, i) => {
        ...
        // each card gets its own dark themed cover palette
        const themes = [
        ...
        return (
          ...
          >
            ...
            <div style={{ position: 'relative', background: '#f5e8cc' }}>
              ...
              {/* scalloped wave — top fill matches image bg, bottom fill matches card bg */}
              <svg
                viewBox="0 0 300 28"
                preserveAspectRatio="none"
              ...
              >
                {/* upper scallops (image color) */}
                <path
                  d="M0,0 Q15,28 30,14 Q45,0 60,14 Q75,28 90,14 Q105,0 120,14 Q135,28 150,14 Q165,0 180,14 Q195,28 210,14 Q225,0 240,14 Q255,28 270,14 Q285,0 300,14 L300,0 Z"
                  fill={t.scallop}
                />
                {/* lower fill (card body color) */}
                <path
                  d="M0,0 Q15,28 30,14 Q45,0 60,14 Q75,28 90,14 Q105,0 120,14 Q135,28 150,14 Q165,0 180,14 Q195,28 210,14 Q225,0 240,14 Q255,28 270,14 Q285,0 300,14 L300,28 L0,28 Z"
                  fill="#fff8ee"
                />
              </svg>
            </div>
            ...
            <div style={{ padding: '0.2rem 1.25rem 1.4rem' }}>
              ...
              {/* title */}
              <div style={{
                color: AMBER_DARK, fontWeight: 700, fontSize: 'clamp(0.95rem,1.8vw,1.08rem)',
              ...
              {allFeatureLabels.map((label, rowIdx) => (
                <tr key={label} style={{ background: rowIdx % 2 === 0 ? CREAM : '#fff' }}>
                  ...
                  {consultation.pricingPlans.map((plan, i) => {
                    ...
                    return (
                      <td key={i} style={{ textAlign: 'center', padding: '1rem 1.25rem', borderRight: i < consultation.pricingPlans.length - 1 ? '2px solid rgba(200,121,26,0.35)' : 'none' }}>
        ...
        <div style={{ maxWidth: 580, margin: '0 auto', position: 'relative', zIndex: 1 }}>
          <div style={{ animation: 'ctaFadeUp 0.6s ease both 0.1s', marginBottom: '1.2rem' }}>
            <div style={{ position: 'relative', display: 'inline-block' }}>
              ...
              <span style={{ fontSize: 'clamp(2.4rem,5vw,3.2rem)', position: 'relative', zIndex: 1 }}>{consultation.icon}</span>
            </div>
          </div>
